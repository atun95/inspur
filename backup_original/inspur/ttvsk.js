...
import { 
  Calendar, Search, ChevronRight, Clock, TrendingUp, Zap, Mail, MapPin, 
  Phone, Image as ImageIcon, Facebook, Linkedin, Youtube, Menu, X, ExternalLink,
...
const App = () => {
  ...
  useEffect(() => {
    const fallbackData = [
      ...
      },
      {
        ...
        date: "10/01/2026",
        time: "6 phút đọc",
        excerpt: "Sự kiện Gala Tất Niên IT-CCTV Tỉnh Đắk Lắk 2025 quy tụ đông đảo doanh nghiệp, đối tác và cộng đồng trong lĩnh vực IT, CCTV và máy văn phòng, với sự tham gia của nhiều nhà tài trợ hàng đầu.",
  ...
  // Nội dung đa ngôn ngữ đơn giản
  const t = useMemo(() => ({
    nav: NAV_LINKS,
    btnPartner: lang === 'vi' ? 'Bắt đầu Hợp tác' : 'Start Partnering'
  }), [lang]);

  const toggleLang = () => setLang(prev => (prev === 'vi' ? 'en' : 'vi'));

  const TextLogo = ({ light = false }) => (
    <div className="flex items-center gap-2 group cursor-pointer" onClick={() => window.location.href = 'https://inspur.com.vn/'}>
      <img
  ...
  useEffect(() => {
    document.title = 'Tin tức và sự kiện - Inspur';
      ...
      <div className={`fixed top-0 right-0 w-[85%] max-w-sm h-full bg-white z-[200] transition-transform duration-500 ease-out lg:hidden shadow-2xl ${
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="p-8 bg-slate-50 space-y-6">
            ...
            <button 
              onClick={() => {
                window.location.href = 'https://inspur.com.vn/contact/';
                setIsMobileMenuOpen(false);
              }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-20 text-left">
            ...
            <div>
              ...
              <ul className="space-y-4 text-sm font-medium text-slate-400">
                {NAV_LINKS.map(l => <li key={l.name}><a href={l.url} className="hover:text-white transition-colors">{l.name}</a></li>)}
              </ul>
            </div>
            <div className="space-y-6">
              ...
              <div className="flex gap-4 text-sm text-slate-400 font-medium">
                <Phone className="text-blue-500 shrink-0" size={20} /> <p>0907 865 892</p>
              </div>
              <div className="flex gap-4 text-sm text-slate-400 font-medium">
